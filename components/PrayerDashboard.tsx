...
// import { PrayerApiResponse } from '../types'; // Not strictly needed if typing is inferred or any
import { MapPin, Clock, Bell, BellOff } from 'lucide-react'; // Removing Calendar as it seemed unused or I can keep it if needed
...
export const PrayerDashboard: React.FC = () => {
  ...
  // Refs to hold latest state for checks
  ...
  const parseTime = (timeStr: string) => {
    ...
    const date = new Date(currentTime); // Use current date
    date.setHours(hours, minutes, 0, 0);
    return date;
  };

  // Check Alarms Effect - Dependent on currentTime from hook (ticks every second)
  useEffect(() => {
    const checkAlarms = () => {
      if (!prayerData || Notification.permission !== 'granted') return;
